...
<template>
  <div>
    <a-modal v-model="openOrg" width="1000px" title="Add Activity">
      ...
      <p>
        ...
        <a-upload-dragger v-if="!data.picture" :file-list="fileList" name="photo" :multiple="false"
          ...
          <p class="ant-upload-hint">
            Support for a single image upload. Strictly upload only image files (jpeg, png, jpg, gif) with a maximum
            size of 2MB.
          </p>
        </a-upload-dragger>
        <a-row type="flex">
          ...
          <a-col class="col-content" :span="24" :xl="12" style="justify-content: center;">
            <a-button type="danger" danger style="width: 100%" v-if="data.picture" @click="handleRemove">Remove
              Image</a-button>
          </a-col>
      ...
      <p>
        <label>Type</label>
        <a-select v-model="data.type" show-search placeholder="Select a type" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
            ">
          <a-select-option v-for="(depart, i) in types" :value="depart" :key="i">
            {{ depart }}
          </a-select-option>
        </a-select>
      </p>
      <p>
        <label>Country</label>
        <a-select v-model="data.pays" show-search placeholder="Select a country" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>State</label>
        <a-select v-model="data.etat" show-search placeholder="Select a state" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>Commune</label>
        <a-select v-model="data.commune" show-search placeholder="Select a commune" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
            ">
          <a-select-option v-for="(depart, i) in communes" :value="depart" :key="i">
            {{ depart }}
          </a-select-option>
        </a-select>
      </p>
      <p>
        <label>Domain</label>
        <a-select v-model="data.domaine" show-search placeholder="Select a domain" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
            ">
          <a-select-option v-for="(depart, i) in domaines" :value="depart" :key="i">
            {{ depart }}
          </a-select-option>
        </a-select>
      </p>
    ...
    <a-modal v-model="openEditOrg" width="1000px" title="Edit Activity">
      ...
      <p>
        ...
        <a-row type="flex">
          ...
          <a-col class="col-content" :span="24" :xl="12" style="justify-content: center;">
            <a-button type="danger" danger style="width: 100%" v-if="editData.picture" @click="handleRemove">Remove
              Image</a-button>
          </a-col>
      ...
      <p>
        <label>Type</label>
        <a-select v-model="editData.type" show-search placeholder="Select a type" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>Country</label>
        <a-select v-model="editData.pays" show-search placeholder="Select a country" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>State</label>
        <a-select v-model="editData.etat" show-search placeholder="Select a state" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>Commune</label>
        <a-select v-model="editData.commune" show-search placeholder="Select a commune" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p>
        <label>Domain</label>
        <a-select v-model="editData.domaine" show-search placeholder="Select a domain" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p v-if="editData.state == 'validated'">
        <label>Status</label>
        <a-select v-model="editData.state" show-search placeholder="Select a status" option-filter-prop="children"
          style="width: 100%" :filter-option="(input, option) =>
              option.componentOptions.children[0].text
                .toLowerCase()
                .includes(input.toLowerCase())
      ...
      <p v-if="editData.state == 'done'">
        <label>Actors</label>
        <a-select v-model="editData.actorIds" mode="multiple" style="width: 100%" placeholder="Select Item..."
          :max-tag-text-length="maxTagTextLength" :options="options"></a-select>
      </p>
      ...
      <template #footer>
        ...
        <a-button key="submit" type="primary" :loading="loadingEdit" @click="handleEditOrg">Submit</a-button>
      </template>
    ...
    <!-- Projects Card -->
    <a-card :bordered="false" class="header-solid h-full mb-24" :bodyStyle="{ paddingTop: '14px' }" style="box-shadow: 0 0 10px rgba(0, 0, 0, .2); padding: 20px;">
      <template #title>
        <a-row type="flex" align="middle">
          ...
          <a-col :span="24" :md="12" style="display: flex; align-items: center; justify-content: flex-end; gap: 4;">

            <a-radio-group v-model="projectHeaderBtns" size="small">
              <a-radio-button value="all" @click="handleAll">ALL</a-radio-button>
              <a-radio-button value="online" @click="handleNotValidate">Not Validated</a-radio-button>
              <a-radio-button value="validate" @click="handleShowValidate">To be achieved</a-radio-button>
              <a-radio-button value="progress" @click="handleShowProgress">In progress</a-radio-button>
              <a-radio-button value="done" @click="handleDone">Done</a-radio-button>
      ...
      <a-row type="flex" align="middle" :gutter="40" style="display: flex  !important; align-items: center !important; justify-content: space-between !important; padding: 0 30px;">

        <a-col :span="24" :md="12" style="margin-top: 10px; margin-bottom: 40px; width: 300px;">
          <div>
        ...
        <a-col :span="24" :md="12" style="margin-top: 10px; margin-bottom: 40px; width: 500px;">
          <a-input-search v-model="search" placeholder="input search text" enter-button @search="handleSearch"
            @input="handleSearch" />
        </a-col>
  ...
  methods: {
    ...
    handleChange(info) {
      ...
      if (status === 'done') {
        ...
        message.success(`Image uploaded successfully. URL: ${this.data.picture}`);
