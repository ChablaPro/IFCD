<template>
  ...
  <a-card :bordered="false" class="header-solid h-full" :bodyStyle="{ padding: 0 }" style="box-shadow: 0 0 10px rgba(0, 0, 0, .2); padding: 20px;">
    <template #title>
      <a-row type="flex" align="middle">
        <a-col :span="24" :md="12">
          <h5 class="font-semibold m-0">Organizations</h5>
        </a-col>
        ...
        <a-col :span="24" :md="12" style="display: flex; align-items: center; justify-content: flex-end; gap: 4;">
          ...
          <a-radio-group v-model="projectHeaderBtns" size="small">
            ...
            <a-radio-button value="online" @click="notValidated">Not Validated</a-radio-button>
    ...
    <a-row type="flex" align="middle" :gutter="40" style="display: flex  !important; align-items: center !important; justify-content: space-between !important; padding: 0 30px;">
      ...
      <a-col :span="24" :md="12" style="margin-top: 10px; width: 500px;">
        <a-input-search v-model="search" placeholder="input search text" enter-button @search="onSearch"
          @input="onSearch" />
      </a-col>

    </a-row>

    <a-table :columns="columns" :data-source="data" :pagination="true" style="margin-top: 25px;">
      ...
      <template slot="editBtn" slot-scope="row">
        <a-space>
          ...
          <a-button type="link" :data-id="row.id" @click="validated(row)" v-if="row.state == 'new'">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
              style="margin-left: 5px">
              <path class="fill-gray-7"
